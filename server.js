...
new Promise(resolve => {
    fs.readFile(`./src/config/fetch.ts`, 'utf-8', (err, data) => {
        if (data.indexOf(`// const bizUrl = "https://m.yourhr.com.cn/zhongyi";`) != -1) {
            console.warn('选择测试场发布代码中...')
            resolve(linkList.filter(item => {
                return item.host == '222.84.159.205'
            })[0])
        }
        if (data.indexOf(`// const bizUrl = "https://sr.cncloud.com/qichang";`) != -1) {
            console.warn('选择正式场发布代码中...')
            resolve(linkList.filter(item => {
                return item.host == '139.198.4.66'
            })[0])
        }
    })
